...
!ech                  - c2*atan(xd) + 1.571_dbl_kind
      ...
      if (sfctype(1:3)=='ice') then
       ...
       ! Alice DuVivier - changes for drag coeffs
            if (TLAT(i,j) > 0) then
              ! Only make roughness change for NH
              !print *,"Changing roughness because ice lat: ",TLAT(i,j)
              vmag(ij) = max(umin, wind(i,j))
              rdn(ij) = sqrt(0.0027_dbl_kind/vmag(ij) + &
                             0.000142_dbl_kind + 0.0000764_dbl_kind*vmag(ij))
              Cdn_atm(i,j) = rd(ij) * rdn(ij)
            else
